...
class MyInventory(Inventory):
    ...
    def __init__(self, resource, loader, variable_manager):
        ...
        print("get_group_dict--->", self.inventory.get_group_dict())
            ...
            if k == "success":
                for x, y in v.items():
                    ...
                    for k, v in data['ansible_devices'].items():
                        if k[0:2] in ['sd', 'hd', 'ss', 'vd', 'xv']:
                            disk = int((int(v.get('sectors')) * int(v.get('sectorsize'))) / 1024 / 1024)
                            disk_size = disk_size + disk
                            if data['ansible_devices'][k]['host']:
                                a = {k: dict(disk_model=data['ansible_devices'][k]['host'],
                                             disk_type=data['ansible_devices'][k]['host'].split()[0],
                                             disk_size=disk / 1024)}
                                cmdb_data['disk_info'] = dict(cmdb_data['disk_info'], **a)
                    cmdb_data['network'] = {}
                    for i in ["ansible_eth0", "ansible_eth1"]:
                        if i in data:
                            if "ipv4" in data[i]:
                                b = {i: dict(name=i.split('_')[1],
                                             ip=data[i]['ipv4']['address'],
                                             netmask=data[i]['ipv4']["netmask"],
                                ...
                                b = {i: dict(name=i.split('_')[1],
                                             mac=data[i]["macaddress"],
                                             status=data[i]["active"])}
                            cmdb_data['network'] = dict(cmdb_data['network'], **b)
                    ...
                    cmdb_data['system'] = data['ansible_system']
                    cmdb_data['system_version'] = data['ansible_distribution'] + ' ' + data[
...
if __name__ == '__main__':
    ...
    for ip in iplist:
        host = ip[0]
        port = models.Server.objects.filter(nic__ipaddrs=host)
        resource.append({"hostname": host, "port": port})
    playbook_file = "/root/djcelery/app/playbook/server_info.yml"
    rbt = ANSRunner(resource, redisKey='1')
    rbt.run_playbook(playbook_path=playbook_file)
    ...
    for k, v in result.get('status').items():
        v['host'] = k
        if 0 < v.get('failed') or v.get('unreachable') > 0:
            v['result'] = 'Failed'
        else:
            v['result'] = 'Succeed'
        dataList.append(v)
    ...
    # print("statPer--->", statPer)
    # print("dataList--->", dataList)
    ANS = ANSRunner(resource)
    ANS.run_model(host_list=["192.168.189.133"], module_name='setup', module_args="")
    # print("result-->", ANS.get_model_result())
